...
```bash
curl -X POST https://api.logstack.io/api/v1/logs/ingest \
...
```bash
curl "https://api.logstack.io/api/v1/logs?projectId=proj_123&level=error&offset=0&limit=50" \
...
```bash
curl https://api.logstack.io/api/v1/logs/log_abc123 \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIs..."
```
