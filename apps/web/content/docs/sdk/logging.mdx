...
Learn how to write effective, searchable, and actionable logs with Logstack.

<Callout type="info">
  <strong>Zero-config collection:</strong> Install the SDK with the default settings and all your existing <code>console.log</code>, <code>console.error</code>, etc. are automatically captured and sent (source: "console"). You get full coverage instantly + the power of structured logging when you use the explicit API.
</Callout>

## Log Levels

Choose the appropriate level for each log entry. Logstack supports five log levels:
