...
description: Configure logstack-js — environments, captureConsole, batching, retries, and more.
---
...
Deep-dive for **logstack-js**. For other languages see [Go configuration](/docs/sdk/go#configuration)
and [Python configuration](/docs/sdk/python#configuration).
...
1. **Vite / modern bundlers** — `import.meta.env.DEV`, `import.meta.env.MODE`, `import.meta.env.PROD`
2. **`process.env.NODE_ENV`** — Node and bundler define (accessed so Vite can replace it)
3. **Local browser hosts** — `localhost`, `127.0.0.1`, `[::1]`, `*.localhost`, `*.local` → `development`
